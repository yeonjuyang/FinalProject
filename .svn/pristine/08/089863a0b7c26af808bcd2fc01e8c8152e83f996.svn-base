...
function emailDetail(emailNo,prevMailNo,nextMailNo){
	   window.location.href = '/mail/detail/deletebox/' + emailNo
...
$(document).ready(function(){
	...
	$("#delBtn").click(function(){
		...
		$.ajax({
			type: "POST",
			traditional:true,
			url: "/mail/hardDelete", // 삭제 요청을 처리할 서버 측 URL
            data: {data : JSON.stringify(deleteDataList)}, // 삭제할 메일 목록을 서버로 보냄
	...
	<div class="wf-mail-right" data-aos="fade-right" data-aos-duration="700" data-aos-delay="400">
		...
		<div class="wf-content-area getmail">
			<div class="wf-flex-box between mg-b-10">
				<h1 class="page-tit2">
					<i class="xi-mail"></i> 휴지통 <span class="mail-badge">${deleteCount}</span>
				</h1>
				<div class="side-util">
					<button type="button" class="btn3" id="delBtn">영구 삭제</button>
				</div>
			</div>
					...
					<tr>
						...
						<th>받은 일시</th>
 										...
 										<tr>
                                            <td class="checkbox-td">
                                                <span class="checkbox-radio-custom">
                                                    <input type="checkbox" name="checkbox" id="chk${stat.count+1}" data-email-no="${emailVO.emailNo}" data-sender-name="${emailVO.senderName}">
							...
							<td> 
								...
								<button type="button">
								<c:choose>
								<c:when test="${empty emailVO.atchmnflNo}"></c:when>
								<c:otherwise>
								<i class="xi-attachment"></i>
											...
											<td onclick="emailDetail('${emailVO.emailNo}', '${emailVO.prevMailNo}', '${emailVO.nextMailNo}')">

...
function emailDetail(emailNo,prevMailNo,nextMailNo){
	   window.location.href = '/mail/detail/main/' + emailNo
...
$(document).ready(function(){
	...
	$("#delBtn").click(function(){
		...
		$.ajax({
			type: "POST",
			url: "/mail/delete/main", // 삭제 요청을 처리할 서버 측 URL
	...
	<div class="wf-mail-right">
		...
		<div class="wf-content-area getmail">
			<div class="wf-flex-box between mg-b-10">
				<h1 class="page-tit2">
					<i class="xi-mail"></i> 받은메일함 <span class="mail-badge">${noreadCount}/${getCount}</span>
				</h1>
			...
			<table class="wf-table">
				<colgroup>
					<col style="width: 2%;"><!-- px로 주기 -->
					<col style="width: 2%;"><!-- px로 주기 -->
					<col style="width: 10%;"><!-- px로 주기 -->
					<col style="width: 10%;">
					...
					<c:forEach var="emailVO" items="${emaildata.content}" varStatus="stat">
						<tr>
							...
							<td>
								...
								<button type="button">
								<c:choose>
								<c:when test="${empty emailVO.cnfirmDate}">
								<i class="xi-mail"></i>
								</c:when>
								<c:otherwise>

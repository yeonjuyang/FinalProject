...
public class ArticlePageForProjectReply<T> {
   // 전체 글 수
   private int total;
   // 현재 페이지 번호
   ...
   // 요청url주소
   private String prjctDutyNo= "";
   // 생성자(Constructor) : 페이징 정보를 생성
   // 753 1 10 select결과 10행
   public ArticlePageForProjectReply(int total, int currentPage, int size, List<T> content, String keyword,String prjctDutyNo) {
      // size : 한 화면에 보여질 목록의 행 수
      this.total = total; // 753
      this.currentPage = currentPage; // 1
      this.content = content;
      this.keyword = keyword;
      this.prjctDutyNo=prjctDutyNo;
      if (keyword==null) {
    	  keyword = "";
      }
      // 전체 글 수가 0이면
      ...
      pagingArea += "<nav class='wf-pagination-wrap'> ";
        ...
        if (this.startPage < 3) { pagingArea += "disabled"; }
        pagingArea += "' href='javascript:getDutyModal(\"" + prjctDutyNo + "\", " + (this.startPage-3)+ ")'>";
        ...
        pagingArea +="'>";
        pagingArea += "<a class='page-link' href='javascript:getDutyModal(\"" + prjctDutyNo + "\", " + pNo + ")'>" + pNo + "</a>";	
        pagingArea += "</li>";
        }
   ...
   public String getPrjctDutyNo() {
	return prjctDutyNo;
}

public void setPrjctDutyNo(String prjctDutyNo) {
	this.prjctDutyNo = prjctDutyNo;
}

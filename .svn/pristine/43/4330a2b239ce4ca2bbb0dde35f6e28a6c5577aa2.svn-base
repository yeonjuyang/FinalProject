...
<script>
    ...
    $(function () {
        ...
        $("#rsvModalCloseBtn").on("click", function () {
            ...
            $("#rsvSTekme").prop("disabled", false);
            $("#rsvETime").prop("disabled", false);
            // 수정삭제버튼 disabled 풀기
            $("#rturnBtn").prop("disabled", false);
        ...
        $("#autoComplete").on("click", function () {
            $("#rsvSDate").val("2024-04-18");
            $("#rsvEDate").val("2024-04-19");
            $("#rsvCont").val("천안지사 출장으로 차량 사용하겠습니다.");
        });
    });
        ...
        $.ajax({
            ...
            success: function (cars) {
                ...
                cars.forEach(function (car, index) {
                    ...
                    if (useCd != "0") {
                        selectCarStr += `<option class="" value="\${car.carNo}">\${car.carNm}</option>`;
                    } else {
    ...
    // 상세 모달을 위한 차량 목록 불러오기
    function getCarsForDetailModal(carNo) {
        $.ajax({
            url: "/api/cars",
            type: "GET",
            dataType: "json",
            success: function (cars) {
                ...
                cars.forEach(function (car, index) {
                    let useCd = car.carUsePosblYnCd;
                    let detailcarNo = car.carNo;
                    selectCarStr += `<option value="\${car.carNo}"`;

                    if (useCd == "0") {
                        selectCarStr += `disabled`;
                    }
                    if (detailcarNo == carNo) {
                        selectCarStr += `selected`;
                    }

                    selectCarStr += `>\${car.carNm}</option>`;

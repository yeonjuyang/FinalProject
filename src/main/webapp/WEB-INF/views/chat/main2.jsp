			...
			success:function(res){
				...
				$.each(res,function(i,v){
					...
					$.ajax({
						...
						success:function(res){
							if(res.mssageCn){
								if(res.mssageCn.trim().substr(0,4) == "<img"){
									res.mssageCn = "(이미지)"
								}
								if(res.mssageCn.trim().substr(0,4) == "<div"){
									res.mssageCn = "(파일)"
								}
...
function getChat(){
	$.ajax({
		...
		success:function(res){
			...
			$.each(res,function(i,v){
				...
				if(v.empNo == empNo){
					str += `<div class="user mine" style="margin-top:10px;">
					...
					str += `<div class="user you" style="margin-top:10px;">
					    <div class="chat-user-info">
					        <div class="chat-img">
					            <img src="/resources/img/icon/avatar.png">
					        </div>
			...
			// 이미지 파일일 경우 기능 넣는 곳
			/* $.each($(".user .chat-txt"),function(i,v){
				if($(v).html().trim().substr(0,4) == "<img"){
					$(v)
				}
			}) */
	...
	$("button[modal-id=modal-chat-modify]").on("click",function(){
		$("#modal-chat-modify .modal-tit").html($(".chatRoom-chat-name").text())
		$("#modal-chat-modify #cc").val("");
	})
	...
	$("#modal-chat-modify .modifyBtn").on("click",function(){
		...
		$.ajax({
			...
			success:function(res){
				getChatRoomList();
				$("#modal-chat-modify").removeClass("open");
				$(".chatRoom-chat-name").html(newChatRoomNm);
			}
...
function send(chatRoomNo) {  // 웹소켓 메세지 전송
	...
	$.each(sockets,function(i,socket) {
		console.log("1:",socket.chatRoomNo)
	...
	$("#upload").on("change",function(){
		...
		$.ajax({
			...
			success:function(res){
				res = res
				let msg = "";
				if(res.substr(res.lastIndexOf(".")) == ".jpg" || res.substr(res.lastIndexOf(".")) == ".jpeg" || res.substr(res.lastIndexOf(".")) == ".png"){
					msg = "<img src='" + _storage + res + "' width='200' height='150'>"
				}else{
					msg =  `<div class="wf-flex-box">
							<button type="button" class="btn1 downloadFileBtn" idx="\${res}" style="height:40px;">
		                    <i class="xi-attachment"></i>
		                    </button>
		                    <div class="wf-content-box" style="width:100px;">\${res.split("_")[1]}</div>
		                    </div>`
				}
				...
				$.each(sockets,function(i,socket){
					if (socket.chatRoomNo == chatRoomNo) {
						...
						$.ajax({
				           	...
				           	success:function(res){
				           		$(`.myChatRoom[idx=\${chatRoomNo}] .chat-date`).html(formattedDateTime.split(" ")[1]);
				           		if(res.split(".")[1] == "jpg" || res.split(".")[1] == "jpeg" || res.split(".")[1] == "png"){
					               	$(`.myChatRoom[idx=\${chatRoomNo}] p[class=chat-txt]`).html("(이미지)")
								}else{
					               	$(`.myChatRoom[idx=\${chatRoomNo}] p[class=chat-txt]`).html("(파일)")

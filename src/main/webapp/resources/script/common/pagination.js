(function(window, jQuery){
	...
	function Pagination({search={}, pagination={}, paginationHTML={}, sort={}}) {
		...
		this.paginationHTML = {
			//frst: (page) => { return `<a href="javascript:void(0);" class="arr frst"><span class="sr-only">처음</span></a>` },
			//prev: (page) => { return `<a href="javascript:void(0);" class="arr prev"><span class="sr-only">이전</span></a>` },
			//prevdot: (page) => { return `<a href="javascript:void(0);" class="arr dot"><span class="sr-only">이전장</span></a>` },
			//page: (page, isActive) => { return `<a href="javascript:void(0);" class="page ${isActive ? 'active' : ''}">${page}</a>`},
			//nextdot: (page) => { return `<a href="javascript:void(0);" class="arr dot"><span class="sr-only">다음장</span></a>`},
			//next: (page) => { return `<a href="javascript:void(0);" class="arr next"><span class="sr-only">다음</span></a>` },
			//last: (page) => { return `<a href="javascript:void(0);" class="arr last"><span class="sr-only">마지막</span></a>` },
		
			frst: (page) => { return `<li class="page-item"><a class="page-link frst" href="#"></a></li>` },
			prev: (page) => { return `<li class="page-item"><a class="page-link prev" href="#"><i class="xi-angle-left"></i></a></li>` },
			prevdot: (page) => { return `<li class="page-item"><a class="page-link dot" href="#">...</a></li>` },
			page: (page, isActive) => { return `<li class="page-item ${isActive ? 'active' : ''}"><a class="page-link" href="#">${page}</a></li>`},
			nextdot: (page) => { return `<li class="page-item"><a class="page-link dot" href="#">...</a></li>`},
			next: (page) => { return `<li class="page-item"><a class="page-link next" href="#"><i class="xi-angle-right"></i></a></li>` },
			last: (page) => { return `<li class="page-item"><a class="page-link last" href="#"></a></li>` },
		};
		this.sort = {};
		/* init search */ 
	...
	var a = new Pagination({
	    search:{
	        ...
	        b:{
	            ...
	            onChangeTargetParam: ['B change'],
	            autoSet: true,
	        },
	    }, 
	    pagination:{
	        pageSize: 5,
	        rowSizeTarget:$('#ROWSIZE'), 
	        pageListTarget:$('#PAGINATION'), 
	        onChangeRowSize: function() {
	            console.log(this, ...arguments);
	        },
	        onChangeRowSizeParam: ['onChangeRowSize'],
	        onClickFrst: function() {
	            console.log(this, ...arguments);
	        },
	        onClickFrstParam: ['onClickFrst'],
	        onClickPrev: function() {
	            console.log(this, ...arguments);
	        },
	        onClickPrevParam: ['onClickPrev'],
	        onClickPrevdot: function() {
	            console.log(this, ...arguments);
	        },
	        onClickPrevdotParam: ['onClickPrevdot'],
	        onClickPage: function() {
	            console.log(this, ...arguments);
	        },
	        onClickPageParam: ['onClickPage'],
	        onClickNextdot: function() {
	            console.log(this, ...arguments);
	        },
	        onClickNextdotParam: ['onClickNextdot'],
	        onClickNext: function() {
	            console.log(this, ...arguments);
	        },
	        onClickNextParam: ['onClickNext'],
	        onClickLast: function() {
	            console.log(this, ...arguments);
	        },
	        onClickLastParam: ['onClickLast'],
	        onClickAll: function() {
	            console.log(this, ...arguments);
	        },
	        onClickAllParam: ['onClickAll'],
	    },
        paginationHTML: {
            prev: function(page) {
                console.log(this, ...arguments);
                return '<a href="javascript:void(0);" class="arr prev"><span class="sr-only">이전</span></a>';
            },
            page: function(page, isActive) {
                console.log(this, ...arguments);
                return `<a href="javascript:void(0);" class="page ${isActive ? 'active' : ''}">${page}</a>`;
            }
        },
        sort: {
            A: {
            ...
            C: {
                ...
                onChangeTargetParam: ['C'],
            },
        },
	}).setPage(1).setRowCount(187).makePagination();
	
	
	function makeSearchTag() {
		var id = this.target.attr('id'),
			tag = $('#SEARCH_TAG_LIST').find(`#TAG_${id}`),
			tagCnt = $('#SEARCH_TAG_LIST').find('li[id*=TAG]').length;
		if(!tag.length) {
			tag = `<li id="TAG_${id}"><span></span><button class="remove-btn"></button></li>`;
			tag = $(tag).find('button').click(this, function (e) {
				e.data.target.val('');
				$(this).closest('li').remove();
			}).closest('li');
			$('#SEARCH_TOGGLE_BTN').addClass('active').find('em.add-num').text(tagCnt + 1);
			$('#SEARCH_TAG_LIST').append(tag);
		}
		tag.find('span').text(this.target.prop('tagName') === 'SELECT' ? this.text : this.value);
		if(!this.value) {
			tag.remove();
			$('#SEARCH_TOGGLE_BTN').find('em.add-num').text(tagCnt-1);
			//if(!(tagCnt-1)) $('#SEARCH_TOGGLE_BTN').removeClass('active');
			if(tagCnt-1 <= 0) $('#SEARCH_TOGGLE_BTN').removeClass('active');
		}
	};
	
	function sortClick() {
		$('th.sort').removeClass('up down');
		switch(this.value) {
			case null:
				this.target.removeClass('up down');
				break;
			case 'ASC':
				this.target.addClass('up');
				break;
			case 'DESC':
